...
export default function initVault({ bridgeETH, bridgeBSC, tokenETH, tokenBSC, TOKEN_DECIMALS=18, TOKEN_SYMBOL='DYP' }) {
    ...
    class Bridge extends React.Component {
        ...
        handleWithdraw = async (e) => {
            ...
            amount = new BigNumber(amount).times(10**TOKEN_DECIMALS).toFixed(0)
            try {
                let url = window.config.SIGNATURE_API_URL+`/api/withdraw-args?depositNetwork=${this.state.network == 'ETH' ? 'BSC': "ETH"}&txHash=${this.state.txHash}`
                console.log({url})
        ...
        refreshBalance = async () => {
            ...
            try {
                ...
                let token_balance = await (network == 'BSC' ? tokenBSC : tokenETH).balanceOf(coinbase)
                ...
                if (this.state.txHash) {
                    try {
                        let url = window.config.SIGNATURE_API_URL+`/api/withdraw-args?depositNetwork=${this.state.network == 'ETH' ? 'BSC': "ETH"}&txHash=${this.state.txHash}&getWithdrawableUnixTimestamp=true`
                        console.log({url})
                                        ...
                                        <div className='col-12'>
                                            <div className='l-box'>
                                                <form onSubmit={this.handleWithdraw} className="pb-0">
                                                    ...
                                                    <p style={{fontSize: '.8rem'}} className='mt-1 text-muted mt-3'>After Successful Deposit, Switch MetaMask to {this.state.network == 'ETH' ? 'BSC' : 'ETH'} network if you deposited on {this.state.network} network!</p>
